...
Sigui $p\in\ZZ$ un primer considerem l'ideal $p\ZZ_K$. Aquest descompon en producte d'ideals primers. Siguin $P_1,\ldots,P_n\subseteq \ZZ_K$ els ideals primers diferents no nuls i $e_1,\ldots,e_n$ els exponents tals que
\[
p\ZZ_K=P_1^{e_1}\cdots P_n^{e_n}.
\]
Com que els ideals no nuls de $\ZZ_K\cong \ZZ^{[K:\QQ]}$ són finitament generats i lliures del mateix rang que $\ZZ_K$ tenim que $\ZZ_K/P_i$ és un domini finit i.e., un cos finit. De fet, se sap que és de característica $p$ ---teorema 27 de \cite{NF}---. En particular, $\#(\ZZ_K/P_i)=p^{f_i}$ per un $f_i\in\ZZ_{\geq 1}$ determinat que anomenarem \emph{grau d'inèrcia} de $p$ dins $P_i$. 
...
\end{thm}
Cal destacar que aquí estem dient, implícitament, que hi ha un nombre finit d'ideals primers amb norma menor o igual a $x\geq 1$.
...
\begin{proof}
    ...
    \[
    \sum_{p\leq x}\frac{\omega_g(p)}{p}=\sum_{d<N(P)\leq x}\frac{1}{N(P)}+\sum_{p\leq d}\frac{\omega_g(p)}{p},
    \]
    on $d\in\mathbb Z_{\geq 1}$ és tal que per $p>d$ el nombre $\omega_g(p)$ equival al nombre d'ideals de norma $p$ de la descomposició en ideals primers de $p\mathbb Z_K$. Aleshores, pels teoremes de Mertens,
    \begin{align*}
