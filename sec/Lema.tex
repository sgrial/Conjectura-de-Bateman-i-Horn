...
Sigui $p\in\ZZ$ un primer considerem l'ideal $p\ZZ_K$. Aquest descompon en producte d'ideals primers. Siguin $P_1,\ldots,P_n\subseteq \ZZ_K$ els ideals primers no nuls i $e_1,\ldots,e_n$ els exponents tals que
\[
p\ZZ_K=P_1^{e_1}\cdots P_n^{e_n}.
\]
Com que els ideals no nuls de $\ZZ_K\cong \mathbb Z^{[K:\QQ]}$ són finitament generats i lliures del mateix rang que $\mathbb Z_K$ tenim que $\ZZ_K/P_i$ és un domini finit i.e., un cos finit. De fet, se sap que és de característica $p$ ---teorema 27 de \cite{NF}---. En particular, $\#(\ZZ_K/P_i)=p^{f_i}$ per un $f_i\in\ZZ_{\geq 1}$ determinat que anomenarem \emph{grau d'inèrcia} de $p$ dins $P_i$. 
